...
class PayU
{
	var $luUrl = "https://secure.payu.ru/order/lu.php", 
		$button = "<input type='submit' class=\"input_button\" value=\"Перейти к оплате\">",
		$debug = 1,
		$showinputs = "hidden",
		$isToken = false,
		$isFrame = false,
		...
		$p2pUrl = "https://secure.payu.ru/order/prepaid/NewCardPayout" ; # https://secure.payu.ua/order/prepaid/NewCardPayout
...
#-----------------------------	
	...
	{
		$str = "";
		if ( !is_array( $val ) ) return '<input type="'.$this->showinputs.'" name="'.$name.'" id="'.$name.'" value="'.htmlspecialchars($val).'">'."\n";
