<?
	...
	$sDocName = '1.5_Protocol_Noise.pdf';
	...
	$sProtokolNum = StringWork::CheckNullStrLite($agroup[sPNumNoiseNoise]);
...
<tr>
<td align="center"><font face="calibrib"><h2>ПРОТОКОЛ № '.$sProtokolNum.'<br />результатов измерения параметров шума и эквивалентных уровней звука</h2></font></td>
</tr>
...
<tr nobr="true"><td align="Left">5. Наименование примененных методов исследований, измерений, нормативно правовых актов регламентирующих нормативные уровни (ПДК/ПДУ):
<font face="calibrib"><ul><li>Приказ Минтруда России №33н от 24 января 2014 г. "Об утверждении методики проведения специальной оценки условий труда, классификатора вредных и (или) опасных производственных факторов, формы отчета о проведении специальной оценки условий труда и инструкции по её заполнению";
</li><li>ГОСТ Р ИСО 9612-2013 "Национальный стандарт Российской федерации. Акустика. Измерения шума для оценки его воздействия на человека.<br>
Метод измерений на рабочих местах", утв. и введен в действие Приказом Росстандарта от 05.12.2013 № 2180-ст;
</li>'.$sOborudMethod.'</ul></font></td></tr></table>';
	//====================================================================================================
	//====================================================================================================
	//====================================================================================================
...
function PDF_InsertHeaderRmList($inPDF)
{
	...
	$inPDF->MultiCell	(98,$h,'2',1,'C',1,0,'','',1,0,0,1,$h,'M');
	...
	$inPDF->MultiCell	(10,$h,'7',1,'C',1,0,'','',1,0,0,1,$h,'M');
	$inPDF->MultiCell	(10,$h,'8',1,'C',1,0,'','',1,0,0,1,$h,'M');
	$inPDF->MultiCell	(10,$h,'9',1,'C',1,0,'','',1,0,0,1,$h,'M');
	$inPDF->MultiCell	(30,$h,'10',1,'C',1,0,'','',1,0,0,1,$h,'M');
	$inPDF->MultiCell	(30,$h,'11',1,'C',1,0,'','',1,0,0,1,$h,'M');
	$inPDF->MultiCell	(17,$h,'12',1,'C',1,1,'','',1,0,0,1,$h,'M');
	//$inPDF->MultiCell	(30,$h,'12',1,'C',1,1,'','',1,0,0,1,$h,'M');
...
function PDF_insert_RM($inPDF, $idWorkGroup, $infontname, $infontnamebold)
{
	...
	$inPDF->MultiCell	(98,$h,'Наименование структурного подразделения, рабочего места; места проведения измерения',1,'C',1,0,'','',1,0,0,1,$h,'M');
//	$inPDF->SetFont($infontname, 'BI', 8, '', 'false');

	$inPDF->StartTransform();

	$inPDF->Rotate(90, 166, 65);
	$inPDF->MultiCell	($h,13,'Стратегия проведения изм. по ГОСТ ИСО 9612-2013*',1,'C',1,0,166,22,1,0,0,1,13,'M');
	$inPDF->MultiCell	($h,13,'Время воздействия, ч.',1,'C',1,0,166,35,1,0,0,1,13,'M');
	$inPDF->MultiCell	($h,13,'Характер / категория**',1,'C',1,0,166,48,1,0,0,1,13,'M');
	$inPDF->MultiCell	($h,13,'Нормативное значение',1,'C',1,0,166,61,1,0,0,1,13,'M');
	$inPDF->StopTransform();

	$inPDF->MultiCell	(30,$h,'Фактическое значение',1,'C',1,0,175,15,1,0,0,1,$h,'M');
	$inPDF->MultiCell	(30,$h,'Эквивалентный уровень звука за 8 часовой рабочий день L EX.8h, дБА',1,'C',1,0,205,15,1,0,0,1,$h,'M');
	$inPDF->MultiCell	(30,$h,'Неопределен-ность измерения, u (L EX.8h)',1,'C',1,0,235,15,1,0,0,1,$h,'M');
	//

	$inPDF->MultiCell	(17,$h,'Итоговый класс',1,'C',1,1,265,15,1,0,0,1,$h,'M');
	...
	$sql = "SELECT `id`, `sName`, `idParent`, `iNumber`, `sNumAnalog`, `fWorkDay`, `iANoise`, `iAInfraNoise`, `iAUltraNoise`, `iAVibroO`, `iAVibroL`, `dEqNoise`, `dSuspNoise` FROM `Arm_workplace` WHERE `idGroup` = ".$idWorkGroup." AND `idParent` > -1 ORDER BY `iNumber`;";
	$vResult = DbConnect::GetSqlQuery($sql);
	$bRow = false;
	if (mysql_num_rows($vResult) > 0)
	{
		while($vRow = mysql_fetch_array($vResult))
		{
			...
			if (mysql_num_rows($vResultF) > 0)
			{
				...
				$rowcount = max($inPDF->getNumLines($tmpUnitName, 98),$inPDF->getNumLines($vRow[iNumber], 10));
				$h = $rowcount*4.5;
				$inPDF->MultiCell	(10,$h,$vRow[iNumber],1,'C',0,0,'','',1,0,0,1,$h,'M');
				$inPDF->MultiCell	(98,$h,$tmpUnitName,1,'L',0,0,'','',1,0,0,1,$h,'M');
				$inPDF->MultiCell	(13,$h,'1',1,'C',0,0,'','',1,0,0,1,$h,'M');
				$inPDF->MultiCell	(26,$h,'-',1,'C',0,0,'','',1,0,0,1,$h,'M');
				//$inPDF->MultiCell	(92,$h,'',1,'C',0,0,'','',1,0,0,1,$h,'M');
				$inPDF->MultiCell	(13,$h,'80',1,'C',0,0,'','',1,0,0,1,$h,'M');
				$inPDF->MultiCell	(30,$h,'-',1,'C',0,0,'','',1,0,0,1,$h,'M');
				$inPDF->MultiCell	(30,$h,$vRow[dEqNoise],1,'C',0,0,'','',1,0,0,1,$h,'M');
				$inPDF->MultiCell	(30,$h,$vRow[dSuspNoise],1,'C',0,0,'','',1,0,0,1,$h,'M');
				$inPDF->MultiCell	(17,$h,$tmpMaxNoise,1,'C',0,1,'','',1,0,0,1,$h,'M');
				//$vRowF[fPdu1]
				$inPDF->SetFont($infontname, 'BI', 10, '', 'false');
				if($num_pages < $inPDF->getNumPages())
				{
					...
					$inPDF->MultiCell	(10,$h,$vRow[iNumber],1,'C',0,0,'','',1,0,0,1,$h,'M');
					$inPDF->MultiCell	(98,$h,$tmpUnitName,1,'L',0,0,'','',1,0,0,1,$h,'M');
					$inPDF->MultiCell	(13,$h,'1',1,'L',0,0,'','',1,0,0,1,$h,'M');
					$inPDF->MultiCell	(26,$h,'-',1,'L',0,0,'','',1,0,0,1,$h,'M');
					//$inPDF->MultiCell	(92,$h,'',1,'C',0,0,'','',1,0,0,1,$h,'M');
					$inPDF->MultiCell	(13,$h,'80',1,'C',0,0,'','',1,0,0,1,$h,'M');
					$inPDF->MultiCell	(30,$h,'-',1,'C',0,0,'','',1,0,0,1,$h,'M');
					$inPDF->MultiCell	(30,$h,$vRow[dEqNoise],1,'C',0,0,'','',1,0,0,1,$h,'M');
				...
				while($vRowF = mysql_fetch_array($vResultF))
				{
					if($tmpZone != $vRowF[sZoneName])
					{
						...
						$inPDF->MultiCell (111,$h,$tmpZone,1,'L',0,0,'','',1,0,0,1,$h,'M');
					...
					switch($vRowF[idFactor])
					{
						case 13:
							...
							PDF_InsertFactor($inPDF, ' - Шум, дБА', $vRowF[var1], $vRowF[var2], $vRowF[var3], $vRowF[fPdu1], '—', $vRowF[sTime], $tmpType);
							//Эквивалент
...
/*
	...
	else
	{
		...
		PDF_insert_EndText($inPDF, '* Ш - широкополосный; Т - тональный; П - постоянный; к, п, и - непостоянный колеблющийся, прерывистый, импульсный.<br />** 1 - На основании рабочей операции, 2 - На основании трудовой функции, 3 - На основании рабочей смены.', $iRmCount, $iRmCountWarning, $sRmCountWarning, $idWorkGroup);
		$inPDF->SetFont($infontname, 'BI', 10, '', 'false');
	}
}


function PDF_InsertFactor($inPDF, $sName, $sVar, $sVar2, $sVar3, $sPdu, $sAsset, $fTime, $sType)
{
	$num_pages = $inPDF->getNumPages();
	$inPDF->startTransaction();
	//Указание зоны
	...
	$h = $rowcount*4.5;
	//$inPDF->MultiCell	(10,$h,'',1,'C',0,0,'','',1,0,0,1,$h,'M');
	//$inPDF->MultiCell	(40,$h,'На основе раб. операции',1,'L',0,0,'','',1,0,0,1,$h,'M');
	$inPDF->MultiCell	(13,$h,$fTime,1,'C',0,0,'','',1,0,0,1,$h,'M');
	$inPDF->MultiCell	(13,$h,$sType,1,'C',0,0,'','',1,0,0,1,$h,'M');
	$inPDF->MultiCell	(13,$h,'-',1,'C',0,0,'','',1,0,0,1,$h,'M');
	$inPDF->MultiCell	(10,$h,$sVar,1,'C',0,0,'','',1,0,0,1,$h,'M');
	$inPDF->MultiCell	(10,$h,$sVar2,1,'C',0,0,'','',1,0,0,1,$h,'M');
	$inPDF->MultiCell	(10,$h,$sVar3,1,'C',0,0,'','',1,0,0,1,$h,'M');
	$inPDF->MultiCell	(77,$h,$sAsset,1,'C',0,1,'','',1,0,0,1,$h,'M');
